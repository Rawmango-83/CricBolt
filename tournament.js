...
function finishMatch(resultText,resultType){
  ...
  DataManager.saveMatch({
    ...
    doubleHattricksInMatch:GameState._matchDoubleHattricks||0,
    tripleHattricksInMatch:GameState._matchTripleHattricks||0,
    quadroHattricksInMatch:GameState._matchQuadroHattricks||0,
    oneFiftiesInMatch:GameState._match150s||0,
    twoHundredsInMatch:GameState._match200s||0,
    twoFiftiesInMatch:GameState._match250s||0,
    threeHundredsInMatch:GameState._match300s||0,
    threeFiftiesInMatch:GameState._match350s||0,
    fourHundredsInMatch:GameState._match400s||0
  });
...
function showWTCStage(){
  ...
  sorted.forEach((t,i)=>h+=`<tr class="${i<2?'qualified':''}">`+
    `<td>${i+1}</td><td>${Security.escapeHtml(CRICKET_TEAMS[t].name)}${t===TournamentState.userTeam?' ?x}?':''}</td>`+
  ...
  Object.entries(sp).forEach(([k,s])=>{
    ...
    s.matches.forEach((m,i)=>{
      if(m.completed){
        ...
        h+=`<div style="padding:4px 0"><button onclick="playWTCMatch('${Security.escapeHtml(k)}',${i})" style="font-size:13px;padding:7px 14px;background:${m.pausedState?'#f59e0b':'#667eea'};color:white;border:none;border-radius:4px;cursor:pointer;width:auto">${m.pausedState?'?? Continue':'??? Match '+m.matchNumber}</button></div>`;
      }
    });
  ...
  if(TournamentState.wtc.final){
    ...
    if(f.completed){
      h+=`<div class="match-card completed"><strong>${Security.escapeHtml(CRICKET_TEAMS[f.team1].name)}</strong> ${f.result.team1Score}/${f.result.team1Wickets} vs <strong>${Security.escapeHtml(CRICKET_TEAMS[f.team2].name)}</strong> ${f.result.team2Score}/${f.result.team2Wickets}<br><span style="color:#38a169">?x?  Champion: ${f.result.winner==='tie'?'DRAW':Security.escapeHtml(CRICKET_TEAMS[f.result.winner].name)}</span></div>`;
    } else {
      h+=`<div class="match-card" onclick="playWTCFinal()"><strong>${Security.escapeHtml(CRICKET_TEAMS[f.team1].name)}</strong> vs <strong>${Security.escapeHtml(CRICKET_TEAMS[f.team2].name)}</strong><br><span style="color:${f.pausedState?'#f59e0b':'#667eea'}">${f.pausedState?'?? Continue':'??? Play Final'}</span></div>`;
    }
  }
...
function showIPLRR(){
  ...
  sorted.forEach((t,i)=>h+=`<tr class="${i<4?'qualified':''}">`+
    `<td>${i+1}</td><td>${Security.escapeHtml(IPL_TEAMS[t].name)}${t===TournamentState.userTeam?' ?x}?':''}</td>`+
  ...
  const tbl=(rows,type)=>{
    ...
    if(type==='bat'){
      ...
      <tbody>${rows.map((p,i)=>`<tr style="background:${i%2?'#f7fafc':'white'}">
        <td style="padding:8px 10px;font-weight:600;color:#667eea">${i===0?'?x?!':i===1?'?x??':i===2?'?x?0':i+1}</td>
    ...
    return`<table style="width:100%;border-collapse:collapse;font-size:13px">
      ...
      <tbody>${rows.map((p,i)=>`<tr style="background:${i%2?'#f7fafc':'white'}">
        <td style="padding:8px 10px;font-weight:600;color:#f5576c">${i===0?'?x?!':i===1?'?x??':i===2?'?x?0':i+1}</td>
        <td style="padding:8px 10px">${Security.escapeHtml(p.name)}</td>
...
const TEAM_BADGES = {
  india:'🇮🇳', australia:'🇦🇺', england:'🏴', pakistan:'🇵🇰', southafrica:'🇿🇦', newzealand:'🇳🇿',
  westindies:'🏝️', srilanka:'🇱🇰', bangladesh:'🇧🇩', afghanistan:'🇦🇫', ireland:'🇮🇪', netherlands:'🇳🇱',
  csk:'🦁', mi:'🔵', rcb:'🔥', kkr:'🟣', srh:'🧡', rr:'💗', dc:'🦅', pbks:'🛡️'
};
...
async function startGame() {
  ...
  if (userPlayers.length < 2) {
    await uiAlert('At least 2 players required for your team.', 'Invalid Team Setup');
    return;
  }
  ...
  GameState.teamPlayers[0] = userPlayers;
  GameState.teamPlayers[1] = oppPlayers;
